...
using namespace std;
//时间计算函数
time_t convert(int year, int mon, int day, int hour, int min, int sec, int week = 0)
{
    tm info;
...
int main()
{
    ...
    string tem;
    getline(std::cin,tem);
    year1 = (tem[0]-'0')*1000 + (tem[1]-'0')*100 + (tem[2]-'0')*10 + (tem[3]-'0')*1;
    ...
    getline(std::cin,tem);
    year2 = (tem[0]-'0')*1000 + (tem[1]-'0')*100 + (tem[2]-'0')*10 + (tem[3]-'0')*1;
    ...
    cout<<abs(convert(year1,mon1,day1,hour1,min1,sec1) - convert(year2,mon2,day2,hour2,min2,sec2)) * 1000;
    return 0;
}